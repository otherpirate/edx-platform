...
@attr(shard=9)
@ddt.ddt
class WordCloudTest(UniqueCourseTest):
    """
    ...
    def setUp(self):
        super(WordCloudTest, self).setUp()
    ...
    def test_word_cloud_is_rendered_with_result(self):
        ...
        """
        expected_data = ['text1', 'text2', 'text3', 'text4', 'text5']
        auto_auth(self.browser, self.USERNAME, self.EMAIL, False, self.course_id)
        self.courseware_page.visit()
        self.assertTrue(self.courseware_page.is_word_cloud_rendered)
        self.courseware_page.input_word_cloud()
        self.courseware_page.save_word_cloud()
        self.assertEqual(len(self.courseware_page.word_cloud_answer_list), 5)
